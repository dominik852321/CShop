...
namespace API.Controllers
{
    public class AccountController : BaseApiController
    {
        ...
        [Authorize]
        [HttpPut("address")]
        public async Task<ActionResult<AddresDto>> UpdateUserAddress(AddresDto address)
        {
            var user = await _userManager.FindByUserByClaimsPrincipleWithAddressAsync(HttpContext.User);
            
            user.Address = _mapper.Map<AddresDto, Address>(address);

            var result = await _userManager.UpdateAsync(user);

            if (result.Succeeded) return Ok(_mapper.Map<Address, AddresDto>(user.Address));
